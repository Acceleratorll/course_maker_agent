    ...
    if course_data.get("modules"):
        ...
        if isinstance(modules_list, list):
            for module in modules_list:
                ...
                if hasattr(module, 'number') and hasattr(module, 'title'):
                    mod_title = f"{module.number}: {module.title}"
                    elements.append(Paragraph(mod_title, style_heading2))
                    elements.append(Paragraph(str(module.achieved), style_body))
                    if hasattr(module, 'lessons') and isinstance(module.lessons, list):
                        for lesson in module.lessons:
                            if (hasattr(lesson, 'number') and hasattr(lesson, 'title') and
                                ...
                                # Process all lesson content sections
                                content_sections = [
                                    ('explanation', lesson.explanation),
                                    ('important_areas', getattr(lesson, 'important_areas', None)),
                                    ('case_study', getattr(lesson, 'case_study', None))
                                ]

                                for section_name, section_content in content_sections:
                                    if section_content:
                                        if section_name != 'case_study':
                                            elements.append(Spacer(1, 6))

                                        # Markdown parsing: * for bullets, ** for bold
                                        import re
                                        content_raw = str(section_content)
                                        content_raw = re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", content_raw)

                                        # Split into lines
                                        lines = [line.strip() for line in content_raw.split('\n') if line.strip()]
                                        bullet_items = []
                                        normal_paragraphs = []
